...
chat_component = """import React, { useState, useEffect, useRef } from 'react';
import { Send, Phone, MessageSquare, ShieldAlert, Truck, PlusSquare, Radio } from 'lucide-react';
...
export default function LiveEmergencyChat({ incidentId, senderRole = 'citizen', senderName = 'Citizen User', targetPhone = '+919876543210' }) {
  ...
  const [targetChannel, setTargetChannel] = useState('driver'); // 'driver' | 'hospital'
  ...
  // Connect WebSocket & fetch chat history
  useEffect(() => {
    if (!incidentId) return;

    // 1. Fetch past chat history
    ...
    try {
      ...
      ws.onerror = (err) => console.log('WS error, fallback to REST:', err);
    } catch (err) {
    ...
    <div className="bg-surface-container rounded-2xl border border-outline-variant/30 flex flex-col h-[380px] shadow-2xl overflow-hidden">
      ...
      <div className="bg-surface-container-high p-3 border-b border-outline-variant/30 flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[10px] text-on-surface-variant">Live Channel: {incidentId}</span>
          </div>
        </div>
      ...
      <div className="px-3 py-1.5 bg-surface-container-high/50 border-t border-outline-variant/20 flex gap-2 overflow-x-auto">
        <button
          ...
          className="px-2.5 py-1 bg-surface-container hover:bg-surface-container-highest rounded-full text-[10px] font-mono text-primary border border-primary/30 flex-shrink-0"
        >
          ?? Share GPS
        </button>
        <button
          onClick={() => handleSend("?? ETA Request: What is the current ambulance arrival time?")}
          className="px-2.5 py-1 bg-surface-container hover:bg-surface-container-highest rounded-full text-[10px] font-mono text-[#FFB900] border border-[#FFB900]/30 flex-shrink-0"
        >
          ?? Request ETA
        </button>
        <button
          onClick={() => handleSend("?? ICU Bed Readiness Confirmed")}
          className="px-2.5 py-1 bg-surface-container hover:bg-surface-container-highest rounded-full text-[10px] font-mono text-[#B084FF] border border-[#B084FF]/30 flex-shrink-0"
        >
          ?? Bed Ready
        </button>
      </div>
      ...
      <div className="p-2.5 bg-surface-container-high border-t border-outline-variant/30 flex gap-2">
        ...
        <button
          onClick={() => handleSend()}
          className="bg-primary text-on-primary p-2 rounded-xl hover:bg-primary/90 transition-all flex items-center justify-center"
        >
          <Send size={16} />
        </button>
      </div>
...
with open("src/components/LiveEmergencyChat.jsx", "w", encoding="utf-8") as f:
    f.write(chat_component)

print("LiveEmergencyChat.jsx component created successfully")
