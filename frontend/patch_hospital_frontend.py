...
hosp_jsx = "src/views/HospitalDashboard.jsx"

new_hosp_jsx = """import React, { useState, useEffect } from 'react';
...
export default function HospitalDashboard() {
  ...
  useEffect(() => {
    const fetchIncidents = async () => {
      try {
        const res = await fetch('/api/hospital/incoming/all');
        if (!res.ok) return;
...
with open(hosp_jsx, "w", encoding="utf-8") as f:
    f.write(new_hosp_jsx)

print("HospitalDashboard patched successfully with live ER queue and resource management")
