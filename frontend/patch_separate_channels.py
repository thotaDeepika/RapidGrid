...
separated_chat_jsx = """import React, { useState, useEffect, useRef } from 'react';
import { Send, Phone, MessageSquare, Radio, MapPin, Clock, CheckCircle2, Truck, PlusSquare } from 'lucide-react';
...
export default function LiveEmergencyChat({ 
  ...
  const [activeChannel, setActiveChannel] = useState('driver'); // 'driver' | 'hospital'
  ...
  // Filter messages for current channel tab
  const filteredMessages = messages.filter(m => {
    if (!m.target_channel) return true; // Legacy fallback
  ...
  const currentTargetPhone = activeChannel === 'driver' ? driverPhone : hospitalPhone;
  const currentTargetName = activeChannel === 'driver' ? 'Ambulance Paramedic Unit' : hospitalName;

  return (
    <div className="bg-surface-container rounded-2xl border border-outline-variant/30 flex flex-col h-[400px] shadow-2xl overflow-hidden">
      {/* Dual Channel Switcher Tabs */}
      <div className="bg-surface-container-high p-2 border-b border-outline-variant/30 flex justify-between items-center gap-2">
        <div className="flex gap-1.5 flex-1">
          <button
            onClick={() => setActiveChannel('driver')}
            className={`flex-1 py-2 px-3 rounded-xl text-xs font-bold font-mono transition-all flex items-center justify-center gap-2 border ${
          ...
          >
            <Truck size={14} />
            <span>PARAMEDIC DRIVER CHAT</span>
          </button>
          ...
          >
            <PlusSquare size={14} />
            <span>HOSPITAL ER DESK CHAT</span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <a
            ...
            className="px-2.5 py-1.5 bg-tertiary/20 text-tertiary hover:bg-tertiary/30 rounded-xl transition-all flex items-center gap-1 text-xs font-bold"
          ...
          <a
            ...
            className="px-2.5 py-1.5 bg-secondary/20 text-secondary hover:bg-secondary/30 rounded-xl transition-all flex items-center gap-1 text-xs font-bold"
      ...
      <div ref={chatStreamRef} className="flex-1 p-3 overflow-y-auto space-y-2 text-xs bg-surface-container-lowest/40">
        {filteredMessages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-on-surface-variant/40 space-y-1">
            ...
            <p className="font-semibold text-xs">No messages in {activeChannel === 'driver' ? 'Paramedic Driver' : 'Hospital ER'} channel.</p>
      ...
      <div className="px-3 py-1.5 bg-surface-container-high/50 border-t border-outline-variant/20 flex gap-2 overflow-x-auto">
        {activeChannel === 'driver' ? (
          <>
            ...
            >
              <MapPin size={10} /> Share GPS Location
            </button>
            <button
              onClick={() => handleSend("ETA Request: What is your current ambulance arrival time?")}
          ...
          <>
            <button
              onClick={() => handleSend("ICU Bed Readiness Inquiry")}
            ...
            >
              <CheckCircle2 size={10} /> Inquire ICU Bed
            </button>
            <button
              onClick={() => handleSend("Patient Arriving Soon - Emergency Prep")}
              className="px-2.5 py-1 bg-surface-container hover:bg-surface-container-highest rounded-full text-[10px] font-mono text-primary border border-primary/30 flex items-center gap-1 flex-shrink-0"
            >
              <PlusSquare size={10} /> Prep ER Bed
            </button>
          </>
        )}
      </div>
      ...
      <div className="p-2.5 bg-surface-container-high border-t border-outline-variant/30 flex gap-2">
        <input
          ...
          placeholder={`Type direct message to ${activeChannel === 'driver' ? 'Paramedic Driver' : 'Hospital ER'}...`}
...
with open(chat_file, "w", encoding="utf-8") as f:
    f.write(separated_chat_jsx)

print("LiveEmergencyChat.jsx updated with separated Paramedic Driver vs Hospital ER channels")
