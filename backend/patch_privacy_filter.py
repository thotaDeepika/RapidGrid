...
# 1. Clear old incidents DB file so lingering test data without citizen ownership is wiped
...
# 2. Update CitizenDashboard.jsx to strictly filter incidents by logged-in citizen credentials
...
# Update incident polling logic to filter strictly by citizen credentials
old_poll_filter = """        // Fetch all active incidents to allow tracking past patient emergencies
  useEffect(() => {
...
new_poll_filter = """  // Fetch active incidents belonging strictly to THIS logged-in citizen
  useEffect(() => {
    const fetchAllIncidents = async () => {
      try {
        const res = await fetch('/api/incidents/');
        ...
        // PRIVACY FILTER: Only include incidents owned by this specific citizen
        const myIncidents = incs.filter(i => {
          if (!citizenInfo?.name) return false;
          const matchName = i.citizen_name && i.citizen_name.toLowerCase() === citizenInfo.name.toLowerCase();
          const matchPhone = i.citizen_phone && i.citizen_phone === citizenInfo.phone;
...
content = content.replace(old_poll_filter, new_poll_filter)

with open(cit_file, "w", encoding="utf-8") as f:
    f.write(content)

print("CitizenDashboard.jsx privacy filter patched successfully")
